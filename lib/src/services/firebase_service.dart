...
class FirebaseService {
  ...
  Future addInit(Goal goal, Strategy strat, String name, String desc) async{
    try{
      ...
      child(strat.key).child("initiatives").push(i.toMap(i));
    }
    catch (error) {
      print(error);
    }
  ...
  Future addDir(Goal goal, Strategy strat, Initiative init,
      String name, String desc, num maxVal) async{
    try{
      Dir d = new Dir(name, desc, maxVal, []);
      await _fbRefGoals.child(goal.key).child("strategies").
      child(strat.key).child("initiatives").child(init.key).
      child("directives").push(d.toMap(d));
    }
    catch (error) {
      print(error);
    }
  ...
  Future changeGoalDescription(Goal goal) async{
    await _fbRefGoals.child(goal.key).child("description").set(goal.description);
  }

  Future changeStratName(Goal goal, Strategy strat) async{
    await _fbRefGoals.child(goal.key).child("strategies").child(strat.key).child("name").set(strat.name);
  }

  Future changeStratDescription(Goal goal, Strategy strat) async{
    await _fbRefGoals.child(goal.key).child("strategies").child(strat.key).child("description").set(strat.description);
  }

  Future changeInitName(Goal goal, Strategy strat, Initiative init) async{
    await _fbRefGoals.child(goal.key).child("strategies").child(strat.key).
    child("initiatives").child(init.key).child("name").set(init.name);
  }

  Future changeInitDescription(Goal goal, Strategy strat, Initiative init) async{
    await _fbRefGoals.child(goal.key).child("strategies").child(strat.key).
    child("initiatives").child(init.key).child("description").set(init.description);
  }

  Future changeDirName(Goal goal, Strategy strat, Initiative init, Dir dir) async{
    await _fbRefGoals.child(goal.key).child("strategies").
    child(strat.key).child("initiatives").child(init.key).
    child("directives").child(dir.key).child("name").set(dir.name);
  }

  Future changeDirDescription(Goal goal, Strategy strat, Initiative init, Dir dir) async{
    await _fbRefGoals.child(goal.key).child("strategies").
    child(strat.key).child("initiatives").child(init.key).
    child("directives").child(dir.key).child("description").set(dir.description);
  }

  Future changeDirMax(Goal goal, Strategy strat, Initiative init, Dir dir) async{
    await _fbRefGoals.child(goal.key).child("strategies").
    child(strat.key).child("initiatives").child(init.key).
    child("directives").child(dir.key).child("max value").set(dir.maxValue);
  }
  ...
  Future deleteInit(String key1, String key2, String key3) async {
    try {
      await _fbRefGoals.child(key1).child("strategies").child(key2).
      child("initiatives").child(key3).remove();
    } catch (e) {
  ...
  Future deleteDir(String key1, String key2, String key3, String key4) async {
    try {
      await _fbRefGoals.child(key1).child("strategies").child(key2).
      child("initiatives").child(key3).child("directives").child(key4).remove();
    } catch (e) {
    ...
    Future signIn() async {
      try {
        await
        _fbAuth.signInWithPopup(_fbGoogleAuthProvider);
      }
      catch (error) {
